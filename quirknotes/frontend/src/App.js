...
function App() {
  ...
  const deleteAllNotes = async () => {
    ...
    try {
      ...
      .then(async (response) => {
          if (!response.ok) {
              alert(`Error trying to delete all notes with status: ${response.status}`)
              console.log("Served failed:", response.status)
          } else {
              await response.json().then((data) => {
                deleteAllNotesState()
      ...
      alert("Error trying to delete all notes")
      console.log("Fetch function failed:", error)
  }
  }
  ...
  const deleteNoteState = (entry) => {
    ...
    setNotes((prevNotes) => [...prevNotes].filter((prevNote) => prevNote._id !== entry._id));
  }
  ...
  const patchNoteState = (_id, title, content) => {
    // Code for modifying state after PATCH all here
    setNotes((prevNotes) => [...prevNotes].filter((prevNote) => prevNote._id !== _id));
    setNotes((prevNotes) => [...prevNotes, {_id, title, content}]);
  }
